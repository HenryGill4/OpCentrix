-- OpCentrix Part Form Refactor - Phase 2: Data Migration (Corrected)
...
WHERE Name IN ('SLS Printing', 'EDM Operations', 'CNC Machining', 'Assembly', 'Finishing') AND IsActive = 1;
...
-- ============================================================================
-- 2.4 Stage Boolean Flag Migration (Using Existing Schema)
-- ============================================================================

BEGIN TRANSACTION;

-- Use the mapping table for automated migration - matching existing PartStageRequirements schema
...
SELECT 
    ...
    lfm.ExecutionOrder,
    COALESCE(p.EstimatedHours / (SELECT COUNT(*) FROM LegacyFlagToStageMap WHERE IsActive = 1), 2.0) as EstimatedHours,
    lfm.DefaultSetupMinutes,
    1 as IsRequired,
    1 as IsActive,
    1 as IsBlocking,
    0 as AllowParallelExecution,
    0 as RequiresSpecificMachine,
    ...
    '' as RequiredTooling,
    'Migrated from legacy boolean flags' as RequirementNotes,
    '' as SpecialInstructions,
    '{}' as StageParameters
FROM Parts p
CROSS JOIN LegacyFlagToStageMap lfm
INNER JOIN ProductionStages ps ON ps.Name = lfm.ProductionStageName AND ps.IsActive = 1
...
-- ============================================================================
-- 2.6 Create Migration Debug View (Drop first if exists)
-- ============================================================================

DROP VIEW IF EXISTS vw_MigratedPartSummary;
...
SELECT 'Final Migration Status' as CheckType;
SELECT * FROM vw_MigratedPartSummary ORDER BY PartNumber;
