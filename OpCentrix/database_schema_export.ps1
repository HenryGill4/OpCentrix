# OpCentrix Database Schema Export Script
# This script extracts the schema from the SQLite databases and creates documentation

Write-Host "?? OpCentrix Database Schema Analysis" -ForegroundColor Cyan
Write-Host "=" * 50
...
$schedulerDbPath = "scheduler.db"

Write-Host "?? Checking for database files..." -ForegroundColor Yellow

if (Test-Path $schedulerDbPath) {
    $dbInfo = Get-Item $schedulerDbPath
    Write-Host "? Found: $($dbInfo.Name) (Size: $([math]::Round($dbInfo.Length / 1KB, 2)) KB)" -ForegroundColor Green
} else {
    Write-Host "? scheduler.db not found" -ForegroundColor Red
    exit 1
}

# Try to extract schema using Entity Framework migrations
Write-Host "?? Extracting schema using dotnet ef..." -ForegroundColor Yellow

try {
    $schemaFile = "scheduler_schema.sql"
    $result = dotnet ef migrations script --output $schemaFile 2>&1
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "? Schema extracted successfully!" -ForegroundColor Green
    } else {
        Write-Host "??  EF migrations failed, creating documentation from context..." -ForegroundColor Yellow
    }
} catch {
    Write-Host "??  EF extraction failed: $($_.Exception.Message)" -ForegroundColor Yellow
}

# Create comprehensive schema documentation
Write-Host "?? Creating schema documentation..." -ForegroundColor Yellow

$timestamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
$dbSize = [math]::Round((Get-Item $schedulerDbPath).Length / 1KB, 2)
...
$docFile = "OpCentrix_Database_Schema_Complete.md"
$schemaDoc | Out-File -FilePath $docFile -Encoding UTF8

Write-Host "? Schema documentation saved to: $docFile" -ForegroundColor Green
...
DATABASE FILE: scheduler.db ($dbSize KB)
TOTAL TABLES: 23 entities in SchedulerContext
...
KEY RELATIONSHIPS:
- Jobs -> Parts (PartNumber)
- Jobs -> Machines (MachineId)
- JobStages -> Jobs (multi-stage workflow)
- MachineCapabilities -> Machines

PERFORMANCE INDEXES:
- Jobs: (MachineId, ScheduledStart)
- Parts: PartNumber (unique)
...
$quickRefFile = "database_quick_reference.txt"
...
Write-Host "Database: scheduler.db ($dbSize KB)"
Write-Host "Tables: 23 entities defined in SchedulerContext"
Write-Host "Files created:"
Write-Host "  - $docFile"
Write-Host "  - $quickRefFile"

Write-Host "`n? Database schema analysis complete!" -ForegroundColor Green